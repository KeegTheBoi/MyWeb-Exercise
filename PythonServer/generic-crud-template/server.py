from generic_crud_manager import GenericCrudManager
...
ENTITY_NAME = "item"  # e.g., "quote", "product"
FILE_PATH_JSON = f"{ENTITY_NAME}s.json"
manager = GenericCrudManager(FILE_PATH_JSON)
...
@app.route(f"/api/{ENTITY_NAME}/<int:item_id>")
...
def create_item():
    ...
    return good_response(f"{ENTITY_NAME.capitalize()} created successfully", manager.read_all())

# API endpoint to update an item
@app.route(f"/api/{ENTITY_NAME}/<int:item_id>", methods=["PUT"])
def update_item(item_id):
    ...
    if updated_item:
        return good_response(f"{ENTITY_NAME.capitalize()} updated successfully", manager.read_all())
    return bad_response(f"{ENTITY_NAME.capitalize()} not found", status_code=404)

# API endpoint to delete an item
@app.route(f"/api/{ENTITY_NAME}/<int:item_id>", methods=["DELETE"])
def delete_item(item_id):
    deleted = manager.delete(item_id)
    if deleted:
        return good_response(f"{ENTITY_NAME.capitalize()} deleted successfully", manager.read_all())
    return bad_response(f"{ENTITY_NAME.capitalize()} not found", status_code=404)
